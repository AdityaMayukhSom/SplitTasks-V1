...
testdata = [
    ("Tony Stark", "tony.stark@avengers.com", "ILovePepperPotts"),
    ("Thor Odinson", "thor.odinson@avengers.com", "JaneFosterOrMjolnir"),
]


def store_user(name: str, email: str, password: str, enabled: bool, session: Session):
    hasher = PasswordHash.recommended()
    user_db = User(full_name=name, email=email, hashed_password=hasher.hash(password), enabled=enabled)
    session.add(user_db)
    session.commit()
...
def test_token_generation_authorized_access(
    name: str, email: str, password: str, client: TestClient, session: Session, jwt_vars: JWTVars
):
    user_db = store_user(name, email, password, True, session)
...
def test_token_generation_user_exists_wrong_password(
    ...
    store_user(name, email, password, True, session)
...
def test_token_generation_user_exists_but_disabled(
    ...
    store_user(name, email, password, False, session)
    payload = {"username": email, "password": password}
    resp = client.post("/token", data=payload, headers=auth_form_headers)
    assert resp.status_code == status.HTTP_401_UNAUTHORIZED
